...
def fetch_and_plot_3d():
    """
    Connects to the Milvus DB, fetches all data, and plots it 
    using a 3-dimensional t-SNE.
    """
    ...
    # 4. Run t-SNE
    print(f"Running t-SNE for 3 dimensions... (this may take a minute)")
    tsne = TSNE(
        n_components=3,  # <-- **** THIS IS THE CHANGE ****
        perplexity=15.0, 
        max_iter=1000, 
        init='pca',
        learning_rate='auto',
    ...
    ax = fig.add_subplot(projection='3d') # Use 3D projection

    for label, color in color_map.items():
        indices = [i for i, l in enumerate(labels) if l == label]
        label_points = embeddings_3d[indices]
        
        # Use ax.scatter() which works for 3D axes
        ax.scatter(
    ...
    ax.set_title('3D t-SNE Visualization of Drone Audio Embeddings', fontsize=18)
    ax.set_xlabel('t-SNE Component 1', fontsize=12)
    ax.set_ylabel('t-SNE Component 2', fontsize=12)
    ax.set_zlabel('t-SNE Component 3', fontsize=12) # Add Z axis label
